...
def add_match_db(match: list, odds_1x2: dict, odds_ou: dict) -> None:
    with sqlite3.connect('matches.db') as conn:
        with closing(conn.cursor()) as cursor:
            ...
            fk_id_visiting_team = add_team_lg_bkmkr('id_team','Teams',match[1])
            fk_id_league = add_team_lg_bkmkr('id_league','Leagues',match[2])
            if not exists([match[3], match[4],fk_id_home_team,fk_id_visiting_team]):
                cursor.execute('''
                ...
                print(f'''\nO jogo {match[0]}x{match[1]} na data: {match[3]}/{match[4]} já existe.''')
...
def add_odds_db(fk_id_match: int, odds_1x2: dict, odds_ou: dict) -> None:
    for i in odds_1x2:
        with sqlite3.connect('matches.db') as conn:
            with closing(conn.cursor()) as cursor:
                fk_id_bookmaker = add_team_lg_bkmkr('id_bookmaker','Bookmakers',i)
                cursor.execute('''
